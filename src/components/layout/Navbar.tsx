...
const LOGO_URL = 'https://i.pinimg.com/originals/f7/97/e0/f797e0d435f74e1b41a49ba08f908d25.png';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16">
          <Link href="/" className="flex items-center gap-2.5 group">
            <img src={LOGO_URL} alt="Bubu & Dudu" className="h-10 w-10 rounded-full object-cover border-2 border-white/30 shadow-md" />
            <span className="font-display font-bold text-xl text-white group-hover:text-cream-100 transition-colors">
              Bubu & Dudu
            </span>
            <span className="hidden sm:inline text-xs font-semibold text-white/60 bg-white/15 px-2 py-0.5 rounded-full">
              crochet
            </span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-5">
            ...
            {session ? (
              <>
                ...
                {isAdmin && (
                  <Link href="/admin" className="text-sm font-semibold text-white bg-white/20 px-3 py-1.5 rounded-full hover:bg-white/30 transition-colors">Admin</Link>
                )}
              ...
              <Link href="/login" className="bg-white text-[#4A90D9] text-sm font-bold px-5 py-2 rounded-full hover:bg-cream-100 transition-colors shadow-md">
                Entrar 💕
              </Link>
            )}
          </div>
          ...
          <div className="md:hidden pb-4 border-t border-white/20 mt-2 pt-4 space-y-3">
            ...
            {session ? (
              <>
                ...
                {isAdmin && <Link href="/admin" className="block text-sm font-semibold text-white py-1" onClick={() => setMenuOpen(false)}>⚙️ Panel Admin</Link>}
