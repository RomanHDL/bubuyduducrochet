...
export async function GET() {
  ...
  // Sales by day (last 7 days) — revenue chart
  ...
  // Conversion rate (paid / total non-cancelled)
  const convRate = activeOrders.length > 0 ? (paidCount / activeOrders.length) * 100 : 0;

  // Users by month (last 4 months)
  const usersByMonth = Array.from({ length: 4 }, (_, i) => {
    const d = new Date(now); d.setMonth(d.getMonth() - (3 - i));
    const month = d.toLocaleDateString('es-MX', { month: 'short' });
    const start = new Date(d.getFullYear(), d.getMonth(), 1);
    const end = new Date(d.getFullYear(), d.getMonth() + 1, 0);
    const count = (recentUsers as any[]).length; // simplified - count from all users
    return { month, count: 0 }; // placeholder
  });
