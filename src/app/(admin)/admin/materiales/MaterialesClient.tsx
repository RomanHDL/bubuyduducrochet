...
export default function MaterialesPage({ initialMaterials }: { initialMaterials?: any[] } = {}) {
  const { data: session, status } = useSession();
  const router = useRouter();
  const cachedMats = getCached<any[]>('/api/materials');
  const seed = (initialMaterials && initialMaterials.length ? initialMaterials : cachedMats) || [];
  if (initialMaterials && initialMaterials.length && !cachedMats) setCached('/api/materials', initialMaterials);
  const [materials, setMaterials] = useState<any[]>(seed);
